...
- 모든 API 스펙은 실제 자바 코드를 작성하기 전에 `openapi.yaml` 파일에 가장 먼저 정의한다.
- 자바 코드 내부에 Swagger 관련 애노테이션을 직접 붙여서 문서화하는 역방향 방식은 사용하지 않는다.
...
- API 스펙 변경 시 자바 코드를 직접 수정하지 않는다. `openapi.yaml`을 먼저 수정하고 재빌드하여 코드를 덮어씌운다.
