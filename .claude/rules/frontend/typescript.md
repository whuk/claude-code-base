---
description: TypeScript 프론트엔드 공통 컨벤션 (Next.js, Vite 공통 적용)
globs: "**/*.ts,**/*.tsx"
---

# TypeScript 프론트엔드 공통 규칙

Next.js, Vite 등 프레임워크와 무관하게 TypeScript 프론트엔드 코드 전반에 적용되는 공통 규칙이다. 프레임워크 고유 규칙은 `nextjs.md`, `vite.md`를 참조한다.
...
- 기능 단위(`features/{feature}/`)로 코드를 묶는다. 각 feature 폴더 내부에 `api/`, `components/`, `hooks/`, `stores/`, `types/` 등 필요한 하위 구조를 둔다.
...
- 라우팅 구조와 feature 폴더를 어떻게 매핑할지는 프레임워크마다 다르므로 `nextjs.md`/`vite.md`를 따른다.
...
- **서버에서 오지 않는 순수 UI 상태**(모달 열림 여부, 사이드바, 테마 등)만 클라이언트 상태 라이브러리(Zustand, 또는 범위가 좁으면 `useState`/Context)로 관리한다.
...
- 재사용 UI 컴포넌트는 npm 패키지로 설치하지 않고 shadcn/ui 방식(Radix 기반 프리미티브 + Tailwind 스타일을 프로젝트 코드로 복사)을 우선 검토한다. 팀은 prop/variant API 설계에 집중한다.
- 컴포넌트 variant는 `class-variance-authority`(CVA)로 선언한다. 컴포넌트 함수 내부가 아닌 모듈 스코프에서 정의해 렌더링마다 재생성되지 않도록 한다.
- 조건부 클래스 및 Tailwind 클래스 충돌 병합은 `clsx` + `tailwind-merge`를 합친 `cn()` 유틸로 처리한다.

## 6. 테스트

- Unit/Integration 테스트는 Vitest + React Testing Library를 사용하고, 테스트 대상 소스와 **같은 디렉토리에 colocate**한다 (`Button.tsx` ↔ `Button.test.tsx`).
...
- 서버에서 가져온 데이터를 Zustand 등 클라이언트 상태 스토어에 그대로 복사해 넣지 않는다.
- 컴포넌트 함수 내부에서 CVA variant 객체를 매 렌더링마다 새로 생성하지 않는다.
