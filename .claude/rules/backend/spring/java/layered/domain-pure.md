...
도메인 패키지 내 클래스를 작성하거나 수정할 때 다음 원칙을 따른다. 이 파일은 SQL-first(ORM 미사용) 또는 WebFlux(R2DBC)를 채택해 Domain을 영속성과 완전히 분리하는 프로젝트에 적용되는 순수 Domain 버전이다.

이 프로젝트가 JPA(ORM)를 채택했다면 이 파일은 적용 대상이 아니다. Domain 클래스가 JPA 엔티티를 겸하는(`@Entity` 마커 + `orm.xml`) 버전이 대신 적용되며, `/rw:init`이 스택에 맞는 한 버전만 남긴다. 이 프로젝트가 Hexagonal(Ports & Adapters)을 채택했다면 이 파일은 적용 대상이 아니다. Layered와 Hexagonal을 한 프로젝트에서 동시에 쓰지 않으므로, 실제로 채택하지 않은 아키텍처 스타일의 규칙 파일은 프로젝트에서 제외한다 (`hexagonal/domain.md` 참조). 마찬가지로 이 프로젝트가 Kotlin을 채택했다면 이 파일은 적용 대상이 아니다. Java와 Kotlin 규칙 파일을 한 프로젝트에서 동시에 쓰지 않으므로, 실제로 채택하지 않은 언어의 규칙 파일도 프로젝트에서 제외한다 (`kotlin/layered/domain-pure.md` 참조).
...
- Domain 클래스는 순수 POJO로 작성한다. JPA·Spring 등 어떤 프레임워크 애노테이션도 갖지 않는다 — `@Entity` 마커조차 허용하지 않는다.
- import 구문에 `jakarta.persistence.*`, `org.springframework.*` 패키지가 등장해서는 안 된다. DB 관련 의존성도 포함하지 않는다.
- Domain은 자신이 영속화되는지, 어떤 저장소를 쓰는지 알지 못한다. 영속성 매핑은 Repository가 전담한다 — DB Row ↔ Domain 변환은 Repository의 명시적 팩토리/생성자를 경유하며, 이 경로도 5번의 불변 조건 검증을 통과한다 (SQL-first는 `repository-sql.md` 2번, WebFlux(R2DBC)는 `repository-r2dbc.md` 2번 참조).
- 별도의 `orm.xml`이나 JPA Entity 클래스를 두지 않는다. Domain 클래스에 영속성 변환 메서드(`toRow()`/`fromRow()` 등)도 두지 않는다(변환은 Repository 전담).
