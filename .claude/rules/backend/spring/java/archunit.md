---
description: 계층 의존 방향과 슬라이스 규율을 ArchUnit 테스트로 강제하는 규칙 (Java, Layered/Hexagonal 공통)
globs: "**/*ArchitectureTest.java"
---

# ArchUnit 아키텍처 테스트 규칙 (Java)
...
이 프로젝트가 Kotlin을 채택했다면 이 파일은 적용 대상이 아니다(`kotlin/archunit.md` 참조). ArchUnit API 자체는 언어 무관이지만, 테스트 코드는 저장소 언어로 작성하므로 언어별 파일을 둔다.
...
`Architectures.layeredArchitecture().consideringAllDependencies()`로 계층을 정의하고 접근 가능 계층을 제한한다. 위반 시 실패한다.
...
- 계층: `controller`/`web`, `service`, `repository`, `domain`. 계층 순서(바깥→안쪽): Controller → Service → Repository → Domain.
- 규칙: 각 계층은 자신보다 안쪽 계층만 접근 가능하고, 안쪽 계층은 바깥 계층을 참조할 수 없다(`layer-communication-rules.md` 6번을 ArchUnit으로 강제). 패키지 정의(`..controller..`, `..service..` 등)는 실제 구조에 맞춰 지정한다.
...
- `@ArchTest` 메서드/필드 단위로 규칙을 나눠 위반 지점을 명확히 드러낸다.
