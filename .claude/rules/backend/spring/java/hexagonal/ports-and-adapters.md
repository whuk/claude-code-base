...
```
domain/
    {Aggregate}.java                 Rich Domain 객체 (domain.md)
application/
  port/
    in/
      {Domain}CommandUseCase.java    인바운드 포트 — Command 흐름 진입점
      {Domain}QueryUseCase.java      인바운드 포트 — Query 흐름 진입점
    out/
      {Domain}CommandPort.java       아웃바운드 포트 — 저장/삭제
      {Domain}QueryPort.java         아웃바운드 포트 — 조회(Projection)
  service/
    {Domain}Service.java             {Domain}CommandUseCase 구현체
    {Domain}Finder.java              {Domain}QueryUseCase 구현체
adapter/
  in/
    web/
      {Domain}Controller.java        생성된 Controller 인터페이스 구현, UseCase 호출
  out/
    persistence/
      {Domain}JpaEntity.java         JPA 매핑 전용 클래스 (domain.md 2번 참조)
      {Domain}JpaRepository.java     Spring Data JPA 인터페이스
      {Domain}PersistenceAdapter.java  {Domain}CommandPort/{Domain}QueryPort 구현체
```
...
- Web Adapter(Controller)가 호출하는 진입점이다. Command/Query 흐름별로 분리한다(`layered/layer-communication-rules.md` 3번의 CQRS 원칙과 동일).
- `{Domain}CommandUseCase`: 생성/수정/삭제 메서드를 선언한다. 파라미터는 Command 객체(4번 참조), 반환값은 생성 시 ID, 수정/삭제 시 `void`.
- `{Domain}QueryUseCase`: 조회 메서드를 선언한다. 파라미터는 Query 객체, 반환값은 Read 전용 객체(`record`).
...
- `service-layer.md`의 Finder/Service 분리, 트랜잭션 선언 규칙을 그대로 따른다. 다만 클래스가 각각 대응하는 UseCase 인터페이스를 `implements`한다.
- `{Domain}Service implements {Domain}CommandUseCase`: 아웃바운드 `{Domain}CommandPort`를 생성자로 주입받아 사용한다.
- `{Domain}Finder implements {Domain}QueryUseCase`: 아웃바운드 `{Domain}QueryPort`를 생성자로 주입받아 사용한다.
...
- `api-dto.md`에서 생성된 Controller 인터페이스를 `implements`한다 (아키텍처와 무관하게 동일).
- Web DTO → Command/Query 변환 후 대응하는 UseCase를 호출한다. 변환 로직 위치는 `layered/layer-communication-rules.md` 4번과 동일한 선택지를 따른다.
