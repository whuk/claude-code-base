...
- **변경 서비스**: `{Aggregate}ModifyService`. 변경 `provided` 포트(`{Domain}Register`, `Enroller`, `CourseCreator` 등)를 구현한다.
- 하나의 서비스가 **여러 역할 포트를 함께 구현**할 수 있다(예: `MemberModifyService implements MemberRegister`가 등록/활성화/수정 역할을 담당). 조회와 변경을 한 클래스에서 겸하지는 않는다.
- 서비스가 조회가 필요하면 같은 애그리거트의 조회 `provided` 포트(`MemberFinder`)를 주입받아 사용한다. 다른 서비스의 구현 클래스를 직접 참조하지 않는다.

## 2. Stereotype 애노테이션과 트랜잭션

- 표준 stereotype 메타 애노테이션을 `support/stereotype` 패키지에 정의해 사용한다:
...
- 도메인 규칙(불변식, 상태 전이)은 Domain에 둔다.
- 조율(orchestration), 중복 검사(`checkDuplicateEmail` 등), 트랜잭션 경계, 외부 연동 호출(`EmailSender`)은 서비스에 둔다.
- 서비스는 Domain 객체를 조회 → Domain 메서드 호출 → 저장 위임 순으로 흐른다. 변경된 Domain 객체를 반환하는 것을 허용한다(`ports-and-adapters.md` 0번 완화 선언).
...
public class MemberModifyService implements MemberRegister {
    private final MemberFinder memberFinder;      // 같은 애그리거트 조회 포트
    private final MemberRepository memberRepository; // required 드리븐 포트
...
- 서비스가 다른 애플리케이션 서비스의 구현 클래스를 직접 주입/참조하지 않는다(`provided` 포트로만).
