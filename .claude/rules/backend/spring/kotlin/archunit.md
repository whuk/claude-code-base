...
- Toby flavor(도메인=엔티티, 애그리거트 간 객체 참조 허용, `domain.md` 6번)에서는 다른 슬라이스에 대해 **조회 메서드만 호출**하도록 커스텀 `ArchCondition`으로 강제한다: 대상이 다른 슬라이스에 속하고 `record`(Kotlin `data class`)/`enum`이 아니면, 호출 메서드명이 `get`/`is`/`ensure`로 시작하는 것만 허용한다.
