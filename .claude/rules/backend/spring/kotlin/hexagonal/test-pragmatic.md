...
- 테스트 설정(`@SpringBootTest`, `@Transactional`, `@AutoConfigureMockMvc` 등)을 개별 테스트 클래스에 직접 선언하지 않는다. `support/stereotype`의 커스텀 애노테이션과 `support/test`의 base class를 사용한다.
- `Thread.sleep` 등 시간 기반 동기화를 사용하지 않는다. 픽스처로 결정적 테스트 데이터를 생성한다.
- 테스트 러너는 JUnit5(+ AssertJ), Mock이 필요하면 Mockito(`mockito-kotlin` 등)를 사용한다.

## 2. 계층별 테스트 전략

### 2.1. Domain 단위 테스트 (Spring 컨텍스트 없음)

- `domain/` 클래스는 순수 JUnit(+ AssertJ)로 테스트한다. 불변 조건, 상태 전이 메서드(`activate()`/`deactivate()`), 도메인 예외 발생 조건을 검증한다.
- 도메인 픽스처는 `{Domain}Fixture`의 함수/`object`로 만든다.
...
- 공용 준비 로직은 `BaseApplicationServiceTest`에 `prepare*` 헬퍼로 두고, 실제 `provided` 포트를 주입해 조합한다.
- `@Transactional`로 테스트마다 롤백되어 상태가 새지 않게 한다.
...
- 컨트롤러 슬라이스만 검증하려면 `@WebMvcTest` + `provided` 포트를 `@MockitoBean`으로 대체한다.
- 전체 흐름을 검증하려면 `@WebApiAdapterTest` stereotype(= `@SpringBootTest` + `@AutoConfigureMockMvc` + `@Transactional`)으로 실제 빈을 통과시킨다.
...
- 어서션 대상 필드도 랜덤에 맡기지 않고 픽스처 또는 커스터마이저(`set`/`supply`)로 명시적으로 고정한다. 어서션이 랜덤 값에 의존하면 안 된다.
...
- Application Service 테스트에서 실제 동작 검증을 회피하려고 모든 Port를 Mock으로 대체하지 않는다(이 flavor는 통합 검증이 기본 — 2.2).
