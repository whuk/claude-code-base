...
이 프로젝트가 MongoDB를 사용하지 않는다면(JPA만 사용) 이 파일은 적용 대상이 아니다. MongoDB 사용 여부는 프로젝트 시작 시점에 고정하므로, 사용하지 않는 프로젝트는 이 파일을 프로젝트에서 제외한다. Layered를 채택했다면 `layered/test-mongodb.md`, Java를 채택했다면 `java/hexagonal/test-mongodb.md`를 참조한다(실제로 채택하지 않은 아키텍처/언어의 규칙 파일은 프로젝트에서 제외한다).
