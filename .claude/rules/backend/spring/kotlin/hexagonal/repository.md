...
- QueryDSL Q-class는 `{Domain}JpaEntity`에 대해 생성된다. `{Domain}JpaEntity`에 `@Entity`가 표준 애노테이션으로 남아 있으므로 kapt에 `JPAAnnotationProcessor`를 등록하면 Q-class 생성이 그대로 동작한다 — orm.xml 우회가 필요 없다(`repository-tools.md` 1.1절 참조).
