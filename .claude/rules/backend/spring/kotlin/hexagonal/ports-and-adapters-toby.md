# Ports & Adapters 구조 규칙 (Hexagonal — Toby flavor, Kotlin)

Controller, Application Service, Domain, Persistence 간의 의존 방향과 데이터 전달 규칙을 정의한다. 이 파일은 Toby(splearn식) flavor의 Kotlin 버전으로, `provided`/`required` 포트 관용구와 역할 기반 세분 포트, Spring Data 리포지토리를 곧 드리븐 포트로 쓰는 실용 헥사고날을 다룬다.
...
이 프로젝트가 Java를 채택했다면 적용 대상이 아니다(`java/hexagonal/ports-and-adapters-toby.md`). Layered/SQL-first/WebFlux를 채택했다면 적용 대상이 아니다(Toby flavor는 Hexagonal + JPA + Spring MVC 전용).

## 0. shared/architecture.md 조항 완화 선언

이 flavor는 `shared/architecture.md`의 다음 조항을 **국소적으로 완화·대체**한다(이 파일이 우선한다):

- **1번·10번(Read 흐름 Read DTO 매핑 / Query에서 Domain 반환 금지)**: Toby는 조회 흐름에서 Domain 객체를 그대로 반환할 수 있다(4번).
- **2번(Command 반환 = 생성 ID / 수정·삭제 Unit)**: Toby는 서비스가 변경된 Domain 객체를 반환하는 것을 허용한다(5번).
...
- 외부 연동 포트(`EmailSender`)·도메인 SPI(`PasswordEncoder`)는 인터페이스만 두고 구현을 adapter가 담당한다. 도구 선택은 `repository.md`(Toby)를 따른다.

## 5. Application Service (구현체)

- `service-layer.md`(Toby)의 분리·트랜잭션·stereotype 규칙을 따른다. 구현체는 슬라이스 루트에 `{Aggregate}{책임}Service`로 둔다. 하나의 서비스가 여러 역할 포트를 구현할 수 있다.
