...
이 프로젝트가 Hexagonal(Ports & Adapters)을 채택했다면 이 파일은 적용 대상이 아니다. Domain 클래스가 JPA 엔티티를 겸하는지(Layered) 완전히 분리되는지(Hexagonal)가 핵심 차이이므로 두 방식을 한 프로젝트에서 동시에 쓰지 않는다 (`hexagonal/domain.md` 참조).
