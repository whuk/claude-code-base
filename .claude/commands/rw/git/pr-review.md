...
1. `gh pr list --head <현재 브랜치> --state open --json number,url,title,baseRefName`으로 현재 브랜치의 열린 PR을 조회합니다. `gh pr view`(인자 없음)는 후보가 여럿이어도 하나만 반환하므로 3항의 확인 절차를 지원하지 못합니다.
...
2. 코멘트 본문은 다음 구조로 작성합니다. **머리말은 2줄, 지적은 한 줄이 기본입니다.**
   ...
   **`파일:라인`** — 3~5줄 설명
   ...
   - **에이전트가 `## 상세`를 반환했으면 그 내용을 `<details>` 블록에 그대로 옮깁니다.** 에이전트가 이미 본문에서 분리해 보낸 근거이므로 다시 압축하거나 버리지 마세요. 4단계 5항에서 커맨드가 직접 내린 근거도 같은 블록에 합칩니다.
...
3. 확인되면 본문을 파일로 쓴 뒤 `--body-file`로 게시합니다. HEREDOC(`--body "$(cat <<'EOF' ...)"`)은 리뷰 본문에 `EOF`만 있는 줄이 포함되면 조기 종료돼 잘린 코멘트가 게시되므로 사용하지 마세요.
   - **본문 파일은 저장소 밖에 씁니다.** 세션 스크래치패드 디렉토리가 있으면 거기에, 없으면 `mktemp`로 만든 경로에 씁니다. 저장소 안에 쓰면 리뷰 산출물이 작업 디렉토리를 더럽히고 다음 커밋에 섞여 들어갑니다.
   - 게시가 끝나면 임시 파일을 삭제합니다.
   ```bash
   BODY=$(mktemp -t pr-review)   # 스크래치패드가 있으면 그 경로를 대신 사용
   # Write 도구로 "$BODY"에 코멘트 본문을 기록한 뒤
   gh pr comment <PR번호> --body-file "$BODY"
   rm -f "$BODY"
   ```
...
- 코멘트 본문 임시 파일을 저장소 안에 쓰거나, 게시 후 지우지 않고 남기기 (5단계 3항)
