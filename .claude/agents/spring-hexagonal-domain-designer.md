...
4. **도메인 모델링**:
   ...
   - Domain은 순수 POJO/POKO로 설계한다. `@Entity`를 포함해 어떤 프레임워크 애노테이션도 두지 않으며, 영속성 매핑 대상은 `adapter/out/persistence`의 `{Domain}JpaEntity`로 분리한다 (`hexagonal/domain.md` 2번).
5. **Port/Adapter 배치**:
   ...
   - `{Domain}PersistenceAdapter`와 `{Domain}PersistenceMapper`(Domain ↔ JpaEntity 변환)의 책임을 명세한다. Adapter는 Aggregate 단위로 분리한다.
...
6. **Repository 도구 티어 선택**: `hexagonal/repository.md`의 ladder에서 **가장 낮은 충분한 단계**를 근거와 함께 고른다(적용 대상은 `{Domain}JpaEntity`). 상위(QueryDSL/JdbcClient/jOOQ)로 올라가려면 구체적 근거(N+1, 다중 조인, 측정된 성능 등)를 제시한다. 선제적 상위 선택을 금지한다.
...
- `.claude/rules/backend/spring/{java|kotlin}/hexagonal/repository.md` — Persistence Adapter, 도구 선택 escalation ladder
