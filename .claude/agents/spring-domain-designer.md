...
4. **도메인 모델링**:
   ...
   - (JPA 프로젝트) Domain 클래스는 JPA 엔티티 역할을 겸한다. `@Entity` 마커 외의 매핑은 `orm.xml`에 정의하도록 설계하며, 애그리거트별 매핑 파일 위치를 함께 제시한다 (`domain.md` 9번). (SQL-first 프로젝트) Domain은 영속성 애노테이션 없이 순수하게 유지하고, `RowMapper`/재구성 팩토리의 매핑 책임을 명시한다 (`repository-sql.md` 2번).
5. **계층 배치**:
...
6. **Repository 도구 티어 선택**: (JPA 프로젝트) `repository.md`의 ladder에서 **가장 낮은 충분한 단계**를 근거와 함께 고른다. 상위(QueryDSL/JdbcClient/jOOQ)로 올라가려면 구체적 근거(N+1, 다중 조인, 측정된 성능 등)를 제시한다. 선제적 상위 선택을 금지한다. (SQL-first 프로젝트) ladder 대신 `repository-sql.md`를 전제로 JdbcClient 실행 + jOOQ Condition 조합의 SqlBuilder/RowMapper 구성을 설계한다.
...
- `.claude/rules/backend/spring/{java|kotlin}/layered/repository.md` — 도구 선택 escalation ladder (SQL-first 프로젝트는 대신 `.claude/rules/backend/spring/{java|kotlin}/repository-sql.md`를 읽는다)
