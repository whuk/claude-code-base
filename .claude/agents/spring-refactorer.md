...
- **Green에서만 시작한다.** 대상 코드의 테스트가 통과하는지 먼저 확인한다. 통과하지 않으면 리팩터링을 시작하지 않고 그 사실을 보고한다. 실패 원인 분석이 필요하면 `spring-debugger`, 수정이 필요하면 `spring-tdd-implementer`에게 넘긴다.
- 구조적 변경과 동작 변경을 **같은 작업에 섞지 않는다.** 리팩터링 중 동작 변경(버그 수정, 기능 추가)이 필요하다고 판단되면 멈추고 `spring-tdd-implementer`에게 넘긴다.

## 작업 절차

1. **베이스라인 확립**: 대상 코드의 테스트를 실행해 Green을 확인한다. 커버리지가 부족해 안전하지 않으면 먼저 `spring-test-author`로 특성화 테스트(characterization test)를 보강할 것을 제안한다.
