...
3. **Red**: 작은 기능 증분을 정의하는 실패 테스트를 먼저 작성한다. 테스트 이름은 동작을 설명한다(`shouldRejectDuplicateEmail`). Domain은 순수 JUnit, Application Service/Finder는 Port를 Mock, Adapter는 `test.md` 기준 base class를 선택한다. 테스트 데이터는 Fixture로 명시적 값을 지정하고(Domain Fixture와 JpaEntity Fixture 겸용 금지), 랜덤/대량 데이터는 Instancio로 생성하되 어서션 대상 필드는 고정한다(`test.md` 4번, Pragmatic flavor면 3번). 경계값은 명시 고정하고 다중 케이스는 `@ParameterizedTest`로 나열한다.
