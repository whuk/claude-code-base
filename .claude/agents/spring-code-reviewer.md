...
아래 규칙 파일은 저장소 언어(Java 또는 Kotlin, 소스 확장자로 판단)에 맞는 `.claude/rules/backend/spring/{java|kotlin}/layered/` 디렉토리에서 읽는다. `api-dto.md`는 언어 공통으로 `spring/` 바로 아래에, `rest-api.md`는 스택 공통으로 `backend/shared/` 아래에 있다.
