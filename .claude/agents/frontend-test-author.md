...
2. **테스트 작성**: 테스트 이름은 동작을 설명한다(`shouldReturnEmptyWhenNoMatch`). **결정적 테스트**로 작성한다 — 실제 타이머나 네트워크 지연에 의존하지 않고, 네트워크/비동기 의존성은 목킹해 결과를 결정적으로 만든다. 크리티컬 경로와 엣지 케이스(경계, null, 예외)를 우선한다.
