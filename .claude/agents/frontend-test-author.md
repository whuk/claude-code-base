...
- 작업 전 `.claude/rules/frontend/typescript.md` 6번을 반드시 읽는다. 테스트 도구 선택과 금지 패턴이 그 규칙에 정의돼 있다.
- 이 저장소가 Next.js인지 Vite인지, 테스트 실행 명령(pnpm/npm)을 파악한다.
- 테스트 대상 코드(컴포넌트/훅/feature)를 먼저 읽고 동작을 이해한 뒤 작성한다.

## 테스트 도구 선택 (typescript.md 6번 기준)
...
- **결정적 테스트**: 실제 타이머나 네트워크 지연에 의존하지 않는다. 네트워크/비동기 의존성은 목킹해 결과를 결정적으로 만든다.
