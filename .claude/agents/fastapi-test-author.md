...
- 작업 전 `.claude/rules/backend/fastapi/fastapi.md` 7번을 반드시 읽는다.
- 이 저장소의 패키지 매니저와 테스트 실행 명령(`pytest`)을 파악한다.
- 테스트 대상 코드(라우터/서비스/리포지토리)를 먼저 읽고 동작을 이해한 뒤 작성한다.

## 테스트 도구 선택 (fastapi.md 7번 기준)
...
- 테스트 이름은 동작을 설명한다(`shouldReturnEmptyWhenNoMatch`, 또는 이 저장소가 snake_case 컨벤션이면 `test_returns_empty_when_no_match`).
