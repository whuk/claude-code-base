...
- 이 저장소가 Next.js인지 Vite인지, 테스트 실행 명령(pnpm/npm)을 파악한다. 작업 전 관련 규칙을 반드시 읽는다:
...
- 이 프로젝트 규칙 위반을 구조적으로 해소하는 리팩터링(예: feature 간 직접 import → shared로 승격, 서버 데이터를 담고 있는 클라이언트 스토어 → TanStack Query로 이전, 컴포넌트 함수 내부에서 매 렌더링마다 생성되는 CVA variant → 모듈 스코프로 추출)은 동작을 바꾸지 않는 범위에서 수행한다.
